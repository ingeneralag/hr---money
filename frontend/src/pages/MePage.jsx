...
const MePage = ({ user, activeSection = "overview" }) => {
  ...
  const currentSection =
    activeSection || location.pathname.split("/").pop() || "overview";
  ...
  const fetchHolidaySettings = useCallback(async () => {
    try {
      ...
      const response = await fetch(
        process.env.REACT_APP_API_URL + "/daily-attendance/holidays",
        {
          headers: {
  ...
  useEffect(() => {
    ...
    newSocket.on("command-sent", (data) => {
      ...
      if (!data.success) {
        setRemoteControlLoading(false);
        alert(
          "فشل في إرسال الأمر: " + (data.error || "التطبيق المكتبي غير متصل")
        );
      }
    });
        ...
        if (timeDiff < 30) {
          ...
          if (statusData.status === "break" || statusData.isOnBreak) {
            ...
            setTimeout(() => {
              if (isOnBreak) {
                console.log(
                  "⏰ المستخدم لا يزال في استراحة - تمديد الحماية لـ 10 دقائق إضافية"
                );
                setTimeout(() => {
                  setBreakProtectionActive(false);
                  console.log(
                    "⏰ تم إلغاء حماية الاستراحة تلقائياً بعد 20 دقيقة إجمالية"
                  );
                }, 600000);
              } else {
                setBreakProtectionActive(false);
                console.log(
                  "⏰ تم إلغاء حماية الاستراحة تلقائياً بعد 10 دقائق"
                );
              }
            }, 600000);
          }
          ...
          console.log(
            "✅ تم استعادة حالة العمل من localStorage بالكامل:",
            statusData
          );
        } else {
    ...
    (screenshots, todayData) => {
      ...
      if (filters.dateRange.start || filters.dateRange.end) {
        ...
        filtered = filtered.filter((screenshot) => {
          ...
          const dateMatch = screenshot.match(
            /screenshot_real_(\d{4}-\d{2}-\d{2})_/
          );
          if (dateMatch) {
            ...
            if (timestampMatch) {
              ...
              console.log(
                `❓ لا يمكن استخراج التاريخ من ${screenshot}, استخدام اليوم الحالي`
              );
            }
          }
      ...
      if (filters.activityType !== "all") {
        filtered = filtered.filter((screenshot, index) => {
          ...
          if (filters.activityType === "work" && productivity >= 70)
            return true;
          if (filters.activityType === "break" && productivity < 30)
            return true;
          if (
            filters.activityType === "idle" &&
            productivity >= 30 &&
            productivity < 70
          )
            return true;
      ...
      if (filters.productivity !== "all") {
        ...
        if (
          filters.productivity === "medium" &&
          (productivity < 50 || productivity >= 80)
        )
          return [];
      ...
      if (filters.timeRange !== "all") {
        filtered = filtered.filter((screenshot) => {
          ...
          if (filters.timeRange === "morning" && hour >= 6 && hour < 12)
            return true;
          if (filters.timeRange === "afternoon" && hour >= 12 && hour < 18)
            return true;
          if (filters.timeRange === "evening" && (hour >= 18 || hour < 6))
            return true;
          return false;
        });
      }
  ...
  const getTodayTrackingData = useCallback(() => {
    ...
    if (desktopTrackingData && desktopTrackingData.todayData) {
      return {
        ...
        screenshots: Array.isArray(desktopTrackingData.todayData.screenshots)
          ? desktopTrackingData.todayData.screenshots
          : [],
      };
    }
  ...
  useEffect(() => {
    ...
    const screenshots = Array.isArray(todayData?.screenshots)
        ...
        if (result.success) {
          ...
          if (savedStatus) {
            try {
              ...
              if (timeDiff < 1800000) {
                ...
                console.log(
                  "🔄 استخدام الحالة المحفوظة بدلاً من البيانات المجلبة:",
                  workStatusInfo
                );
              }
            } catch (e) {
          ...
          if (shouldUpdateStatus) {
            ...
            if (breakProtectionActive || isOnBreak) {
              console.log(
                "🛡️ حماية إضافية: منع تحديث الحالة بسبب حماية الاستراحة أو الحالة الحالية"
              );
            ...
            console.log(
              "🔄 تحديث الحالة من البيانات المجلبة (بدون استراحة تلقائية):",
              workStatusInfo
            );
  ...
  const verifyDataConsistency = async (desktopData) => {
    if (
      !dailyRecordsData?.records ||
      dailyRecordsData.records.length === 0 ||
      !desktopData
    )
      return;
    ...
    try {
      ...
      console.log("🔍 فحص تطابق البيانات:", {
        ...
        جدول_تفصيلي_مُنسق:
          todayRecord.totalFormatted || formatTime(recordSeconds),
      });
      ...
      if (savedStatus) {
        try {
          ...
          if (timeDiff < 1800000) {
            shouldFetch = false;
            console.log(
              "⏸️ تم تخطي جلب بيانات التتبع لحماية الحالة المحفوظة حديثاً"
            );
          }
        } catch (e) {
  ...
  const startBreakWithReason = (reason) => {
    ...
    setRemoteControlLoading(false);
    showNotification(
      `☕ تم بدء الاستراحة: ${reason || "بدون سبب محدد"}`,
      "info"
    );
  };
  ...
  const fetchEmployeeProfile = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        process.env.REACT_APP_API_URL + `/employees/profile/${user.id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const fetchPerformanceData = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        process.env.REACT_APP_API_URL + `/employees/performance/${user.id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const fetchAttendanceData = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        process.env.REACT_APP_API_URL + `/employees/attendance/${user.id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const fetchSalaryData = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        process.env.REACT_APP_API_URL + `/employees/salary/${user.id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const fetchDocumentsData = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        process.env.REACT_APP_API_URL + `/employees/documents/${user.id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const fetchRequestsData = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        process.env.REACT_APP_API_URL + `/employees/requests/${user.id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const fetchNotificationsData = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        process.env.REACT_APP_API_URL + `/employees/notifications/${user.id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const fetchStatsData = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        process.env.REACT_APP_API_URL + `/employees/stats/${user.id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
  ...
  const fetchBenefitsData = useCallback(async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(
        process.env.REACT_APP_API_URL + `/employees/benefits/${user.id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        if (result.success && result.data?.records) {
          ...
          const formattedData = result.data.records.map((record) => {
            ...
            const totalHours = record.totalSeconds
              ? record.totalSeconds / 3600
              : record.totalHours || 0;
            const activeHours = record.activeSeconds
              ? record.activeSeconds / 3600
            ...
            return {
              ...
              day:
                record.day ||
                recordDate.toLocaleDateString("ar", { weekday: "long" }),
        ...
        if (result.success) {
          ...
          const employeeData = {
            ...
            location:
              ...
              result.data.workLocation ||
              result.data.address ||
              "",
            allowances: {
              transport:
                result.data.allowances?.transportation ||
                result.data.allowances?.transport ||
                result.data.benefits?.transportationAllowance ||
                0,
              food:
                result.data.allowances?.meal ||
                result.data.allowances?.food ||
                result.data.benefits?.mealAllowance ||
                0,
              housing:
                result.data.allowances?.housing ||
                result.data.benefits?.housingAllowance ||
                0,
              performance:
                result.data.allowances?.performance ||
                result.data.benefits?.performanceAllowance ||
                0,
              ...result.data.allowances,
            },
            deductions: {
              insurance:
                result.data.deductions?.socialInsurance ||
                result.data.deductions?.insurance ||
                0,
              taxes:
                result.data.deductions?.tax ||
                result.data.deductions?.taxes ||
                0,
              loans:
                result.data.deductions?.loans ||
                result.data.deductions?.loan ||
                0,
              absence: result.data.deductions?.absence || 0,
              ...result.data.deductions,
            },
            monthlyAdjustments: result.data.monthlyAdjustments || {
              bonuses: [],
              deductions: [],
            },
          };
  ...
  useEffect(() => {
    if (user?.id) {
      ...
      const fetchAllData = async () => {
        try {
          ...
          await Promise.allSettled([
            fetchEmployeeProfile().catch((err) =>
              console.warn("خطأ في جلب الملف الشخصي:", err)
            ),
            fetchPerformanceData().catch((err) =>
              console.warn("خطأ في جلب بيانات الأداء:", err)
            ),
            fetchAttendanceData().catch((err) =>
              console.warn("خطأ في جلب بيانات الحضور:", err)
            ),
            fetchDesktopTrackingData().catch((err) =>
              console.warn("خطأ في جلب بيانات التتبع:", err)
            ),
            fetchSalaryData().catch((err) =>
              console.warn("خطأ في جلب بيانات الراتب:", err)
            ),
            fetchDocumentsData().catch((err) =>
              console.warn("خطأ في جلب المستندات:", err)
            ),
            fetchRequestsData().catch((err) =>
              console.warn("خطأ في جلب الطلبات:", err)
            ),
            fetchNotificationsData().catch((err) =>
              console.warn("خطأ في جلب الإشعارات:", err)
            ),
            fetchDailyRecords().catch((err) =>
              console.warn("خطأ في جلب السجلات اليومية:", err)
            ),
            fetchStatsData().catch((err) =>
              console.warn("خطأ في جلب الإحصائيات:", err)
            ),
            fetchBenefitsData().catch((err) =>
              console.warn("خطأ في جلب بيانات المزايا:", err)
            ),
            fetchHolidaySettings().catch((err) =>
              console.warn("خطأ في جلب إعدادات الإجازات:", err)
            ),
          ]);
        } catch (error) {
      ...
      fetchAllData();
      fetchHolidaySettings().catch((err) =>
        console.warn("خطأ في جلب إعدادات الإجازات عند التحميل:", err)
      );
      ...
      const trackingInterval = setInterval(() => {
        ...
        if (savedStatus) {
          try {
            ...
            if (timeDiff < 1800000) {
              shouldUpdate = false;
              console.log(
                "⏸️ تم تخطي تحديث التتبع لحماية الحالة المحفوظة حديثاً"
              );
            }
          } catch (e) {
        ...
        if (shouldUpdate) {
          fetchDesktopTrackingData().catch((err) =>
            console.warn("خطأ في تحديث بيانات التتبع:", err)
          );
        }
      }, 5000);
      ...
      const dataInterval = setInterval(() => {
        fetchRequestsData().catch((err) =>
          console.warn("خطأ في تحديث الطلبات:", err)
        );
        fetchNotificationsData().catch((err) =>
          console.warn("خطأ في تحديث الإشعارات:", err)
        );
        fetchStatsData().catch((err) =>
          console.warn("خطأ في تحديث الإحصائيات:", err)
        );
        fetchDailyRecords().catch((err) =>
          console.warn("خطأ في تحديث السجلات اليومية:", err)
        );
      }, 300000);
  ...
  const fetchDailyRecords = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.success) {
          console.log(
            "✅ تم جلب السجلات اليومية من DailyAttendance بنجاح:",
            result
          );
          setDailyRecordsData({
  ...
  const syncMonthRecords = useCallback(async () => {
    ...
    try {
      ...
      const result = await dailyAttendanceService.syncMonth(
      ...
      if (result.success) {
        ...
        await fetchDailyRecords();
        showNotification(
          "تم تحديث جميع سجلات الشهر من بيانات التطبيق",
          "success"
        );
      } else {
        console.warn("⚠️ فشل في تحديث سجلات الشهر:", result.message);
        showNotification(
          "فشل في تحديث سجلات الشهر: " + result.message,
          "error"
        );
      }
    } catch (error) {
  ...
  const employeeData = {
    ...
    department:
      employeeProfile?.employee?.department || "تقنية المعلومات والتطوير",
    email:
    ...
    address:
      employeeProfile?.employee?.address ||
      "شارع التحرير، وسط البلد، القاهرة، جمهورية مصر العربية",
    joinDate:
      employeeProfile?.employee?.joinDate ||
      employeeProfile?.employee?.startDate ||
      "2023-03-15",
    employeeId: employeeProfile?.employee?.employeeId || "EMP-2024-001",
    directManager:
      employeeProfile?.employee?.directManager ||
      "محمد أحمد - مدير التطوير التقني",
    workLocation:
      employeeProfile?.employee?.workLocation ||
      "مكتب القاهرة الرئيسي - الطابق الثالث - قسم التطوير",
    salary: {
    ...
    notifications: notificationsData?.notifications || [
      ...
      },
      {
        ...
        message: "موعد التقييم الربع سنوي قريباً",
        time: "5 ساعات",
        type: "warning",
        read: true,
      },
      {
        ...
        message: "دورة Excel المتقدم متاحة الآن",
        time: "1 يوم",
        type: "info",
        read: true,
      },
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start space-y-4 md:space-y-0 md:space-x-6 rtl:space-x-reverse">
            ...
            <div className="flex-1 text-center md:text-right">
              <h1 className="text-3xl font-bold mb-2">
                مرحباً، {employeeData.name}
              </h1>
              <p className="text-blue-100 text-lg">
                {employeeData.position} - {employeeData.department}
              </p>
              <p className="text-blue-200 text-sm mt-2">
                رقم الموظف: {employeeData.employeeId}
              </p>
            ...
            <div className="text-center md:text-left">
              <div className="bg-white bg-opacity-20 rounded-lg p-3">
                ...
                <p className="text-xs text-blue-200">
                  دخول: {employeeData.attendance.checkInTime}
                </p>
              </div>
            </div>
          </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-green-700 dark:text-green-300">
                    {" "}
                    {formatCurrency(employeeData.salary.net)}{" "}
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-blue-700 dark:text-blue-300">
                    {employeeData.performance.attendance}%
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-purple-700 dark:text-purple-300">
                    {employeeData.performance.rating}/5
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-teal-700 dark:text-teal-300">
                    {employeeData.performance.completed}/
                    {employeeData.performance.tasks}
                  </p>
                </div>
                    ...
                    >
                      ...
                      <div className="text-sm text-gray-600 dark:text-gray-300">
                        ...
                        {request.duration && request.duration !== "-" && (
                          <p>المدة: {request.duration}</p>
                        )}
                        {request.description && (
                          <p>الوصف: {request.description}</p>
                        )}
                        {request.approvedBy && (
                          <p>تمت الموافقة من: {request.approvedBy}</p>
                        )}
                      </div>
                    </div>
                  );
        ...
        <div className="relative z-10 flex items-center justify-between">
          <div className="flex items-center space-x-4 rtl:space-x-reverse">
            ...
            <div className="relative flex items-center">
              ...
              <div
                className={`w-5 h-5 ${statusColor} rounded-full absolute animate-ping opacity-20`}
              ></div>
              <div
                className={`w-7 h-7 ${statusColor} rounded-full absolute animate-ping opacity-10`}
            ...
            <div>
              ...
              <div className={`text-sm ${textColor} opacity-80 mt-1`}>
                {currentStatus === "working" && (
                  <div className="flex items-center space-x-4 rtl:space-x-reverse">
                    <span>
                      🕐 وقت العمل اليوم:{" "}
                      {formatTime(
                        desktopTrackingData?.todayData?.totalSeconds || 0
                      )}
                    </span>
                    <span>
                      🎯 الإنتاجية:{" "}
                      {desktopTrackingData?.todayData?.productivity || 0}%
                    </span>
                  </div>
                )}
                {currentStatus === "break" && isOnBreak && (
                  <div className="flex items-center space-x-4 rtl:space-x-reverse">
                    ...
                    {breakStartTime && (
                      <span>
                        🕐 بدأت الساعة:{" "}
                        {breakStartTime.toLocaleTimeString("ar-EG")}
                      </span>
                    )}
                  </div>
                )}
                {currentStatus === "paused" && (
                  <span>
                    ⏸️ العمل متوقف مؤقتاً - يمكنك الاستكمال أو الإنهاء
                  </span>
                )}
          ...
          <div className="flex items-center space-x-4 rtl:space-x-reverse">
            {/* معلومات مبسطة */}
            <div
              className={`text-sm ${textColor} text-center p-3 rounded-lg bg-white bg-opacity-20 backdrop-blur-sm border border-white border-opacity-30`}
            >
  ...
  const renderDesktopTracking = () => {
    ...
    const isAppConnected =
      desktopTrackingData?.data?.isConnected ||
      ...
      <div className="space-y-6">
                    ...
                    onClick={async () => {
                      try {
                        ...
                        if (response.ok) {
                          ...
                          if (result.success) {
                            showNotification(
                              "تم تحديث سجلات اليوم بنجاح",
                              "success"
                            );
                            await fetchDailyRecords();
                          } else {
                            showNotification(
                              "فشل في تحديث السجلات: " + result.message,
                              "error"
                            );
                          }
                        } else {
            ...
            <CardContent>
              <div className="space-y-3">
                ...
                <Button
                  ...
                  title={
                    !desktopAppConnected
                      ? "يجب تشغيل تطبيق سطح المكتب أولاً"
                      : ""
                  }
                >
                  {realTodayData.isWorking
              ...
              <div className="space-y-3">
                <div className="text-center">
                  <span className="text-sm text-purple-600 dark:text-purple-300">
                    استخدم التطبيق المكتبي للحصول على أفضل تجربة تتبع
                  </span>
                </div>
                  ...
                  <span className="font-medium text-purple-600 dark:text-purple-400">
                    {(() => {
                      const totalBreakSeconds =
                        (realTodayData.breakSeconds || 0) +
                        (isOnBreak ? breakDuration : 0);
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              تتبع ساعات العمل والإنتاجية خلال الأسبوع الماضي
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <Tooltip
                  formatter={(value, name) => {
                    if (name === "productivity")
                      return [`${value}%`, "نسبة الإنتاجية"];
                    if (name === "workTime")
                      return [`${Math.round(value / 3600)} ساعة`, "وقت العمل"];
                    if (name === "activeTime")
                      return [`${Math.round(value / 3600)} ساعة`, "وقت النشاط"];
                    return [value, name];
                  }}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              يتم أخذ لقطات عشوائية كل فترة لضمان الحضور الفعلي - البيانات محمية
              ولا يمكن مسحها
            </CardDescription>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateFilter("activityType", e.target.value)
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateFilter("productivity", e.target.value)
                      }
                      className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 text-sm"
                    >
                ...
                <div className="flex justify-between items-center mt-4 pt-4 border-t border-gray-200 dark:border-gray-600">
                  <div className="flex items-center space-x-2 rtl:space-x-reverse">
                    ...
                    <span className="text-xs text-gray-500 dark:text-gray-400">
                      {filteredScreenshots.length} من{" "}
                      {realTodayData.screenshots?.length || 0} صورة
                    </span>
                  </div>
                  <Button
                    size="sm"
                  ...
                  {totalImages > 0 && (
                    <div className="flex justify-between items-center mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                      <div className="text-sm text-blue-800 dark:text-blue-200">
                        ...
                        {totalPages > 1 && <span className="mx-2">|</span>}
                        {totalPages > 1 && (
                          <span>
                      ...
                      {totalPages > 1 && (
                        <div className="text-xs text-blue-600 dark:text-blue-300">
                          عرض {startIndex + 1}-{Math.min(endIndex, totalImages)}{" "}
                          من {totalImages}
                        </div>
                      )}
                    </div>
        ...
        <Card className="bg-gradient-to-br from-green-50 to-emerald-100 dark:from-green-900/20 dark:to-emerald-800/20 border-green-200 dark:border-green-700">
          <CardHeader>
            ...
            <CardDescription className="text-green-600 dark:text-green-400">
              🔗 مربوط مباشر مع تطبيق سطح المكتب - البيانات متطابقة 100% مع
              التطبيق (إجمالي، نشاط، خمول، استراحة)
              {!dailyRecordsData && (
              ...
              {dailyRecordsData &&
                ...
                !dailyRecordsData.records.some((r) => r.hasRealData) && (
                  <div className="mt-2 p-2 bg-blue-100 dark:bg-blue-900/20 rounded text-blue-800 dark:text-blue-200 text-sm">
                    ℹ️ يتم عرض البيانات الافتراضية - تأكد من تشغيل التطبيق
                    المكتبي
                  </div>
                )}
            </CardDescription>
                    ...
                    >
                      ...
                      <td className="border border-gray-300 dark:border-gray-600 px-3 py-2 text-center text-sm font-medium">
                        {(() => {
                          ...
                          const totalSeconds =
                            record.totalSeconds ||
                            (record.totalHours ? record.totalHours * 3600 : 0);
                      ...
                      <td className="border border-gray-300 dark:border-gray-600 px-3 py-2 text-center text-sm font-medium">
                        {(() => {
                          // إعطاء أولوية للبيانات المُنسّقة مسبقاً
                          if (
                            record.activeFormatted &&
                            record.activeFormatted !== "0 دقيقة"
                          ) {
                      ...
                      <td className="border border-gray-300 dark:border-gray-600 px-3 py-2 text-center text-sm font-medium text-yellow-600 dark:text-yellow-400">
                        {(() => {
                          // إعطاء أولوية للبيانات المُنسّقة مسبقاً
                          if (
                            record.idleFormatted &&
                            record.idleFormatted !== "0 دقيقة"
                          ) {
                          ...
                          const idleSeconds =
                            record.idleSeconds ||
                            (record.idleHours ? record.idleHours * 3600 : 0);
                      ...
                      <td className="border border-gray-300 dark:border-gray-600 px-3 py-2 text-center text-sm font-medium text-orange-600 dark:text-orange-400">
                        {(() => {
                          // إعطاء أولوية للبيانات المُنسّقة مسبقاً
                          if (
                            record.breakFormatted &&
                            record.breakFormatted !== "0 دقيقة"
                          ) {
                          ...
                          const breakSeconds =
                            record.breakSeconds ||
                            (record.breakHours ? record.breakHours * 3600 : 0);
                      ...
                      <td className="border border-gray-300 dark:border-gray-600 px-3 py-2 text-center text-sm font-medium">
                        {(() => {
                          if (record.isWeekend || record.status === "إجازة")
                            return "-";
                          const requiredSeconds = 8 * 3600; // 8 ساعات مطلوبة
                          const totalSeconds =
                            record.totalSeconds ||
                            record.totalHours * 3600 ||
                            0;
                          ...
                          return (
                            <span className="text-orange-600">
                              {formatHoursToHoursMinutes(remainingHours)}
                            </span>
                          );
                        })()}
                      </td>
                        ...
                        {(() => {
                          ...
                          if (record.totalSeconds && record.activeSeconds) {
                            productivity = Math.round(
                              (record.activeSeconds / record.totalSeconds) * 100
                            );
                          }
                ...
                <div className="text-2xl font-bold text-purple-600 dark:text-purple-400">
                  {(() => {
                    ...
                    const activeSeconds = records.reduce((sum, r) => {
                      return (
                        sum + (r.activeSeconds || r.activeHours * 3600 || 0)
                      );
                    }, 0);
              ...
              <Button
                onClick={async () => {
                  ...
                  try {
                    ...
                    await fetchDailyRecords();
                    showNotification(
                      "تم تحديث جميع البيانات من التطبيق",
                      "success"
                    );
                  } catch (error) {
                ...
                <div>
                  ...
                  <div className="bg-white dark:bg-gray-800 rounded-lg p-3 mb-3 border-l-4 border-green-500">
                    <p className="text-sm text-green-700 dark:text-green-300 font-medium">
                      🎯 <strong>البيانات متطابقة 100%:</strong> لو التطبيق يظهر
                      4 دقائق وقت كلي و 2 دقيقة نشط، ستجد نفس الأرقام هنا في
                      الجدول
                    </p>
                  </div>
                  <ul className="text-sm text-green-700 dark:text-green-300 space-y-1">
                    <li>
                      • <strong>إجمالي الوقت:</strong> نفس الرقم الموجود في
                      التطبيق تماماً
                    </li>
                    <li>
                      • <strong>وقت النشاط:</strong> نفس وقت النشاط في التطبيق
                    </li>
                    <li>
                      • <strong>وقت الخمول:</strong> نفس وقت الخمول المحسوب في
                      التطبيق
                    </li>
                    <li>
                      • <strong>وقت الاستراحة:</strong> نفس وقت الاستراحة إذا تم
                      أخذ فترات راحة
                    </li>
                    <li>
                      • <strong>زر "تحديث اليوم":</strong> يحدث سجل اليوم الحالي
                      فقط من بيانات التطبيق
                    </li>
                    <li>
                      • <strong>زر "تحديث الشهر":</strong> يحدث جميع سجلات الشهر
                      الحالي من بيانات التطبيق
                    </li>
                    <li>
                      • <strong>مؤشر ✅ تطبيق:</strong> يظهر للأيام التي تحتوي
                      على بيانات حقيقية من التطبيق
                    </li>
                  </ul>
                </div>
              </div>
                ...
                <div className="flex items-start space-x-3 rtl:space-x-reverse">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-amber-700 dark:text-amber-300 mt-1">
                      نظام المراقبة مصمم لضمان العدالة والشفافية في بيئة العمل.
                      جميع البيانات تُستخدم لأغراض إدارية فقط ومحمية بأعلى
                      معايير الخصوصية. في حالة وجود أي استفسارات، يرجى التواصل
              ...
              <div className="p-4">
                <div className="w-full h-96 bg-gray-200 dark:bg-gray-700 rounded-lg flex items-center justify-center mb-4 overflow-hidden">
                  ...
                  <div
                    ...
                    style={{
                      display: selectedScreenshot.url ? "none" : "block",
                    }}
                  >
  ...
  const generateDailyRecords = () => {
    ...
    if (
      ...
      console.log(
        "📊 استخدام السجلات اليومية الحقيقية من API مع تطبيق إعدادات الإجازات:",
        dailyRecordsData.records.length,
        "سجل"
      );
      ...
      const records = dailyRecordsData.records.map((record) => {
        ...
        const activeSeconds =
          record.activeSeconds ||
          (record.activeHours ? record.activeHours * 3600 : 0);
        const idleSeconds =
          record.idleSeconds ||
          (record.idleHours ? record.idleHours * 3600 : 0);
        const breakSeconds =
      ...
      console.log("✅ تم تنسيق البيانات للعرض - عينة من البيانات:", {
        ...
        recordsWithData: records.filter(
          (r) => r.totalSeconds > 0 || r.hasRealData
        ).length,
      });
    ...
    for (let i = 13; i >= 0; i--) {
      ...
      if (isWeekend) {
        ...
        const breakHours = realTodayData.breakSeconds
          ? realTodayData.breakSeconds / 3600
          : 0;
  ...
  const renderSalary = () => {
    ...
    const baseSalary =
      employeeProfile?.baseSalary || employeeProfile?.salary || 200;
    ...
    const workingDaysOnly = attendanceDataEmployee.filter((day) => {
      ...
      console.log(
        `📅 اليوم ${day.date}: isWorkingDay=${isWorkingDay}, status=${day.status}, isWeekend=${day.isWeekend}, totalHours=${day.totalHours}`
      );
      return isWorkingDay;
    });
    ...
    const totalLateDays = workingDaysOnly.filter((day) => {
      ...
      console.log(
        `🕐 يوم ${day.date}: actualHours=${actualHours}, requiredHours=${requiredHours}, isLate=${isLate}`
      );
      return isLate;
    }).length;
    ...
    const totalLateHours = workingDaysOnly.reduce((sum, day) => {
      ...
      const actualHours = day.totalHours || 0;
      const lateHours =
        actualHours > 0 && actualHours < requiredHours
          ? requiredHours - actualHours
          : 0;

      console.log(
        `⏰ يوم ${day.date}: actualHours=${actualHours}, lateHours=${lateHours}`
      );
    ...
    const workingDaysInMonth =
      workingDaysOnly.length > 0 ? workingDaysOnly.length : 22; // افتراض 22 يوم عمل إذا لم توجد بيانات
    ...
    const totalMonthlyDeductions = monthlyDeductions.reduce(
    ...
    const netSalary =
      employeeData.salary.basic + totalBonuses - totalAllDeductions;
              ...
              <div className="flex items-center space-x-3 rtl:space-x-reverse">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                    إدارة الراتب والمزايا
                  </h2>
                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                    إدارة شاملة لراتب الموظف والمكافآت والخصومات
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-3xl font-bold text-blue-700 dark:text-blue-300">
                    {formatCurrencyLocal(employeeData.salary.basic)}
                    {/* {alert(formatCurrencyLocal(baseSalary))} */}
                  </p>
                  <p className="text-xs text-blue-500 dark:text-blue-400 mt-1">
                    المبلغ الثابت الشهري
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-3xl font-bold text-green-700 dark:text-green-300">
                    {formatCurrencyLocal(totalBonuses)}
                  </p>
              ...
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  ...
                  <p className="text-3xl font-bold text-red-700 dark:text-red-300">
                    {formatCurrencyLocal(totalAllDeductions)}
                  </p>
                  <div className="text-xs text-red-500 dark:text-red-400 mt-1 space-y-1">
                    <div>
                      خصومات شهرية:{" "}
                      {formatCurrencyLocal(totalMonthlyDeductions)}
                    </div>
                    <div>
                      خصم التأخير: {formatCurrencyLocal(totalLatenessDeduction)}
                    </div>
                  </div>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="text-center p-6 bg-white dark:bg-gray-800 rounded-lg border-2 border-yellow-200 dark:border-yellow-700 hover:shadow-lg transition-all duration-300">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
                  ساعات التأخير الشهري
                </p>
                <p className="text-2xl font-bold text-yellow-700 dark:text-yellow-300">
                  {formatHoursToHoursMinutes(totalLateHours)}
                </p>
                <p className="text-xs text-yellow-500 dark:text-yellow-400 mt-1">
                  إجمالي في {totalLateDays} يوم
                </p>
              </div>
              ...
              <div className="text-center p-6 bg-white dark:bg-gray-800 rounded-lg border-2 border-orange-200 dark:border-orange-700 hover:shadow-lg transition-all duration-300">
                ...
                <p className="text-2xl font-bold text-orange-700 dark:text-orange-300">
                  {formatCurrencyLocal(hourlyRate)}
                </p>
              ...
              <div className="text-center p-6 bg-white dark:bg-gray-800 rounded-lg border-2 border-red-200 dark:border-red-700 hover:shadow-lg transition-all duration-300">
                ...
                <p className="text-2xl font-bold text-red-700 dark:text-red-300">
                  {formatCurrencyLocal(totalLatenessDeduction)}
                </p>
                ...
                <div className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {formatCurrencyLocal(baseSalary)}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>أيام العمل الفعلية (بدون عطل):</span>
                    <span className="font-medium">
                      {workingDaysInMonth} يوم
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>الراتب اليومي (÷ أيام العمل):</span>
                    <span className="font-medium">
                      {formatCurrencyLocal(Math.round(dailyRate))}
                    </span>
                  </div>
                  <div className="flex justify-between">
          ...
          <CardHeader>
            <CardTitle className="flex items-center justify-between">
              <div className="flex items-center space-x-3 rtl:space-x-reverse">
                ...
                <div>
                  <span>
                    سجل التأخيرات الشهري - {getArabicMonthName(selectedMonth)}
                  </span>
                  <div className="mt-2 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-700">
                    <div className="text-sm text-green-700 dark:text-green-300 flex items-center">
                      <span className="flex items-center">
                        ...
                        البيانات تأتي من نفس مصدر{" "}
                        <strong>جدول سجل الأيام التفصيلي</strong> في صفحة
                        Desktop Tracking
                      </span>
                    </div>
                    <p className="text-sm text-green-600 dark:text-green-400 mt-1">
                      أوقات الحضور الكلي والنشط ثابتة من التطبيق - حساب ساعات
                      التأخير والخصم يعتمد على 8 ساعات يومياً (بدون احتساب العطل
                      الرسمية والإجازات)
                    </p>
                  </div>
                </div>
            ...
            {loadingAttendance ? (
              <div className="text-center py-16">
                ...
                <p className="text-amber-600 dark:text-amber-400 text-lg font-medium">
                  جاري تحميل بيانات التأخيرات من الخادم...
                </p>
                <p className="text-gray-500 dark:text-gray-400 text-sm mt-2">
                  يتم جلب البيانات الحقيقية لشهر{" "}
                  {getArabicMonthName(selectedMonth)}
                </p>
              </div>
              ...
              <div className="text-center py-16">
                ...
                <p className="text-gray-400 dark:text-gray-500 text-sm mt-2">
                  سيتم إنشاء البيانات تلقائياً عند الحاجة
                </p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full table-auto border-collapse">
                  <thead>
                    <tr className="bg-amber-100 dark:bg-amber-900/30">
                      <th className="border border-amber-200 dark:border-amber-700 px-4 py-3 text-right text-sm font-semibold text-amber-800 dark:text-amber-200">
                        التاريخ
                      </th>
                      <th className="border border-amber-200 dark:border-amber-700 px-4 py-3 text-center text-sm font-semibold text-amber-800 dark:text-amber-200">
                        اليوم
                      </th>
  ...
  const renderAttendance = () => {
    ...
    // حساب الإحصائيات
    const workingDays = monthlyAttendance.filter(
      (day) => day.status !== "عطلة"
    );
    const presentDays = workingDays.filter(
      (day) => day.status === "حاضر" || day.status === "متأخر"
    );
    ...
    const totalOvertime = presentDays.reduce(
      (sum, day) => sum + day.overtime,
      0
    );

    const attendanceRate = Math.round(
      (presentDays.length / workingDays.length) * 100
    );
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-center">
                  <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg">
                    ...
                    <p className="font-bold text-blue-700 dark:text-blue-300">
                      {employeeData.attendance.checkInTime}
                    </p>
                  </div>
                  <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg">
                    ...
                    <p className="font-bold text-gray-700">
                      {employeeData.attendance.checkOutTime}
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-purple-50 dark:bg-purple-900/20 p-3 rounded-lg text-center">
                  ...
                  <p className="font-bold text-purple-700 dark:text-purple-300">
                    {employeeData.attendance.totalHours} ساعة
                  </p>
                </div>
              </div>
            </CardContent>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                آخر تقييم: {formatDate(employeeData.performance.lastReview)}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="text-center p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                    ...
                    <div className="text-lg font-bold text-blue-700 dark:text-blue-300">
                      {employeeData.performance.achievedGoals}
                    </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2 rtl:space-x-reverse text-sm">
                      ...
                      <span className="text-gray-700 dark:text-gray-300">
                        تطوير نظام التقارير المالية
                      </span>
                    </div>
                    <div className="flex items-center space-x-2 rtl:space-x-reverse text-sm">
                      <CheckCircle className="w-4 h-4 text-green-500 dark:text-green-400" />
                      <span className="text-gray-700 dark:text-gray-300">
                        قيادة مشروع تحسين العمليات
                      </span>
                    </div>
                    <div className="flex items-center space-x-2 rtl:space-x-reverse text-sm">
                      <CheckCircle className="w-4 h-4 text-green-500 dark:text-green-400" />
  ...
  const renderTabContent = () => {
    switch (currentSection) {
      ...
      case "desktop-tracking":
        return renderDesktopTracking();
      case "salary":
        return renderSalary();
      case "attendance":
        ...
        >
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-md w-full mx-4">
            ...
            <p className="text-gray-600 mb-4">
              اكتب سبب أخذ الاستراحة (اختياري):
            </p>
            <textarea
              value={breakReason}
            ...
            {breakNotes.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-500 mt-2">
                  ستظهر هنا ملاحظات الاستراحات التي تسجلها
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                <div className="text-sm text-gray-600 mb-4">
                  إجمالي الاستراحات المسجلة:{" "}
                  <span className="font-semibold">{breakNotes.length}</span>
                </div>
                    ...
                    .map((note) => (
                      ...
                      >
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <button
                            onClick={() => {
                              const updatedNotes = breakNotes.filter(
                                (n) => n.id !== note.id
                              );
                              setBreakNotes(updatedNotes);
                              localStorage.setItem(
                ...
                <div className="flex justify-center mt-6">
                  <button
                    onClick={() => {
                      if (
                        window.confirm(
                          "هل أنت متأكد من حذف جميع ملاحظات الاستراحة؟"
                        )
                      ) {
                        setBreakNotes([]);
